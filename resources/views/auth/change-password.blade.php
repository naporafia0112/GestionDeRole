...
<head>
    <meta charset="utf-8" />
    <title>Changement de Mot de Passe - DIPRH</title>
    ...
    <style>
        :root {
            ...
            --text-color: #1e293b;
            --light-text: #64748b;
            --success-color: #10B981; /* Vert pour le succès */
        ...
        .btn-success { /* Nouveau style pour le bouton de changement de mot de passe */
...
<body class="flex items-center justify-center p-4">
    ...
    <div class="w-full max-w-md animate__animated animate__fadeIn">
        <div class="login-card p-8 sm:p-10">
            <div class="text-center mb-8">
                ...
                <h1 class="text-2xl font-bold mt-4 text-gray-800">Sécurisez votre compte DIPRH</h1>
                <p class="text-gray-500 mt-2">C'est votre première connexion ou votre mot de passe doit être mis à jour. Veuillez définir un nouveau mot de passe.</p>
            </div>
            ...
            <form method="POST" action="{{ route('password.change.update') }}" class="space-y-6">
                @csrf
                @method('put') {{-- Important pour la méthode updateForcedPassword du contrôleur --}}
                ...
                <div class="flex items-center justify-end mt-4">
                    <button type="submit"
                            class="btn-primary w-full flex justify-center py-3 px-4 border border-transparent rounded-lg shadow-sm text-white font-medium focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-green-500">
    ...
    <script>
        // Toggle password visibility (adaptée pour plusieurs champs)
        ...
        // Show errors with SweetAlert if any - ADAPTÉ pour le contexte de cette page
        @if ($errors->any())
        ...
        // Afficher le message de succès avec SweetAlert si présent
        @if(session('status'))
            document.addEventListener('DOMContentLoaded', function() {
                Swal.fire({
                    icon: 'success',
                    title: 'Succès !',
                    text: '{{ session('status') }}',
                    confirmButtonColor: '#10B981', // Utilisez la couleur verte pour le succès
                    background: '#ffffff',
                    backdrop: `
                        rgba(16, 185, 129, 0.1)
                    `
        ...
        // Add animation to form elements on load (peut être ajusté si trop d'éléments animés)
        document.addEventListener('DOMContentLoaded', function() {
